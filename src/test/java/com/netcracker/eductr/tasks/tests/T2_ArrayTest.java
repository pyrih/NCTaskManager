...
public class T2_ArrayTest {
    ...
    public void part2_testRemove() {
        ...
        // remove unexistent
        if (isOld())
            Assert.assertFalse("ArrayTaskList.remove(D) не повинно бути істинним", actual.remove(new Task("F", 42)));
        else
            Assert.assertFalse("ArrayTaskList.remove(D) не повинно бути істинним", actual.remove(new Task("F", FROM_NOW_42)));
